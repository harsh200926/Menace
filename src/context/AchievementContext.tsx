...
export const AchievementProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    setAchievements(prev => {
      return prev.map(achievement => {
        if (achievement.id === achievementId && !achievement.completedAt) {
          const newProgress = Math.min(achievement.progress + incrementBy, achievement.total);
  ...
  const checkDailyStreaks = () => {
    ...
    if (todosData) {
      ...
      const allCompleted = hasDailyTodos && todos.every((todo: any) => 
    ...
    if (habitsData) {
      ...
      const dailyHabits = habits.filter((habit: any) => habit.frequency === "daily");
      const allCompleted = dailyHabits.length > 0 && 
        dailyHabits.every((habit: any) => 
    ...
    if (journalData) {
      ...
      const hasTodayEntry = journal.some((entry: any) => 
    ...
    setPoints(prev => {
      ...
      if (!lastUpdate) {
        ...
        if (diffDays === 1) {
          ...
          if (streak % 7 === 0) {
            // Weekly bonus
            addXP(streak * 10, `${type.charAt(0).toUpperCase() + type.slice(1)} Weekly Streak`);
          } else {
            addXP(10, `${type.charAt(0).toUpperCase() + type.slice(1)} Daily Streak`);
          }
        } else if (diffDays > 1) {
          // Streak broken
          streak = 1;
